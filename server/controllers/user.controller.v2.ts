...
import { createUser, queryUsers, updateUserById, deleteUserById, getUserInfoById } from '../services/user.service';
...
export const getUserController = catchAsync(async (req: Request, res: Response) => {
  if (typeof req.params['userId'] === 'string') {
    const user = await getUserInfoById(new mongoose.Types.ObjectId(req.params['userId']));
    if (!user) {
