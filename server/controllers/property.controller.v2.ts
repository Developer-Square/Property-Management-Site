...
export const getPropertiesController = catchAsync(async (req: Request, res: Response) => {
  const filter = pick(req.query, ['propertyType', 'propertyStatus', 'title_like']);
...
export const getPropertyController = catchAsync(async (req: Request, res: Response) => {
  if (typeof req.params['propertyId'] === 'string') {
    const property = await getPropertyById(new mongoose.Types.ObjectId(req.params['propertyId']));
    if (!property) {
